...
- **Customer booking form's map picker migrated off Leaflet/Photon (2026-09-18)**: `CustomerRequestDelivery.jsx`'s `LocationPickerModal` previously rendered with `react-leaflet`/OSM tiles and used Photon (Komoot) for search-as-you-type and click/drag reverse-geocoding — undocumented until now, and in direct contradiction with this section's own Places API (New) provisioning (`lib/reverseGeocode.js`'s comment even assumed "Places Autocomplete on the booking form" was already true, which it wasn't). Now uses `GoogleMap`/`Marker`/`Polygon` from `@react-google-maps/api` for rendering, the new **Places API (New)** `AutocompleteSuggestion`/`AutocompleteSessionToken` classes for search (session-tokened, biased — not restricted — to the Luzon bbox via `locationBias`), and the Maps JavaScript API's `Geocoder` class for click/drag reverse-geocoding (same technique `lib/reverseGeocode.js` already used elsewhere). `'places'` added to `lib/googleMapsLoaderOptions.js`'s shared `GOOGLE_MAPS_LIBRARIES`. All Luzon-only service-area enforcement (`lib/serviceArea.js`'s `isInsideLuzon`/`snapToLuzon`, `SERVICE_AREA_MAX_BOUNDS`) is unchanged — that logic is pure lat/lng math, not tied to either mapping library. Note this does **not** touch `DriverDeliveries.jsx`'s own Leaflet map (the contradiction referenced just above) or `lib/forwardGeocode.js`'s Photon-based stop-text fallback (kept deliberately — see that file's comment on a real Google mis-geocode case it was built to avoid), both out of scope for this change.
...
**Geocoder/router mismatch fix (2026-09-17)**: both `estimateLegDurations` and `computeSuggestedRoute` route the stop's Photon-resolved *coordinate* through Google's `DirectionsService` — since Photon (OSM) and Google don't share a road graph, a coordinate real and accurate to Photon can still be a point Google's routing has no path to (commonly an interior street of a gated subdivision), returning `ZERO_RESULTS`/`NOT_FOUND` for an address that's genuinely deliverable. Both functions now retry once with the leg's raw address text instead of its coordinate whenever DirectionsService fails with one of those two statuses (`RETRYABLE_DIRECTIONS_STATUSES`), letting DirectionsService's own geocoding find a connected point near the same place; Photon's coordinate is still what's stored/displayed, this retry only affects the routing request. Applies to the schedule preview, the route preview above, and `handleSubmit`'s own recompute below. See `STATUS.md`'s 2026-09-17 entry for the full root-cause writeup and verification status.
