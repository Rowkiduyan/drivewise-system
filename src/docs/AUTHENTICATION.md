...
- **Admin-created accounts** (`AdminHome.jsx`, any role — Supervisor/Admin/Driver/Helper/Customer): created via the `admin-users` Edge Function's `create-user` action, which uses `supabase.auth.admin.createUser` (server-side, `service_role`) and then inserts the matching `users` row itself. This runs entirely outside the admin's own browser session — the admin's session is never touched during creation. When `role` is `Driver`, the function also inserts a matching `driver_records` row in the same request via the shared `ensureDriverRecord` helper (`auth_id` = the new user's id, `id` = the next sequential `D001`/`D002`/... value, `first_name`/`last_name` best-effort split from the same email-derived name used for `users.full_name`, `middle_name`/`birthdate`/`profile_picture` left null). If that insert fails, both the `users` row and the auth user are rolled back so a Driver account is never left without its `driver_records` row.
