...
```
    ...
    **Tightened 2026-08-30** (`20260830130000_harden_driver_default_assignments.sql`, `SUPABASE_GOTCHAS.md` #12). Originally built as an explicit copy of the trucks table's pre-hardening model — `for all to authenticated using(true) with check(true)` — per its own migration's comment. Confirmed live and exploitable via curl (an authenticated Driver token could insert/read/update/delete any row); replaced with a `current_user_role() = ANY (ARRAY['Admin','Supervisor'])`-gated policy, verified fixed via the same curl reproduction (insert now returns `403`, Postgres code `42501`).
    ...
    - suggested_route (jsonb, nullable) — **deployed 2026-08-14** (`supabase/migrations/20260814131307_delivery_requests_suggested_route.sql`). Shape: an array of per-leg entries, `[{"from": "pickup", "to": "dropoff", "path": [[lat,lng], ...]}, ...]`, each `path` built from `DirectionsResult`'s per-step `path` (denser than `overview_path`). See `IMPLEMENTATION/11_ROUTE_COMPARISON.md`. **Generation point changed 2026-09-06** (Supervisor Route Review & Approval feature): originally written once by the Driver app's `save-suggested-route` action the first time the pre-trip screen computed it; now generated for every request at customer-creation time (`CustomerRequestDelivery.jsx`, via `lib/suggestedRoute.js`'s `computeSuggestedRoute`), reviewable and editable by a Supervisor during `PENDING_REQUEST` review (`SupDeliveries.jsx`'s `EditableRouteMap`) before a quotation is submitted. Not frozen by mere presence anymore — see `route_approved_at` below. The Driver's pre-trip screen (`PlannedRouteMap`) still recomputes and overwrites this column whenever `route_approved_at` is unset, same fallback role it always had.
    - route_approved_at (timestamptz, nullable) — **deployed 2026-09-06** (`supabase/migrations/20260906000000_delivery_requests_route_approval.sql`). Set when a Supervisor explicitly approves the auto-generated/edited `suggested_route` during `PENDING_REQUEST` review, before submitting a quotation. Once set, `suggested_route` is frozen: `driver-trip`'s `save-suggested-route` action and `DriverDeliveries.jsx`'s `PlannedRouteMap` both key their skip-recompute logic off this column, not off `suggested_route`'s mere presence, since every request now has a `suggested_route` from creation time regardless of whether anyone reviewed it.
    - route_approved_by (uuid, nullable, references `users(id)`) — **deployed 2026-09-06**, same migration as above. The Supervisor who approved the route, same convention as `delivery_quotations.submitted_by`.
