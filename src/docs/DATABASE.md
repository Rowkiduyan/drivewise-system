    ...
    **Created 2026-08-12 via the Storage REST API** (`POST {SUPABASE_URL}/storage/v1/bucket`, service-role key) rather than the Dashboard — unlike raw table DDL (which needs the SQL Editor/a DB connection), bucket creation goes through the same REST surface `service_role` already has, so no manual step was needed this time. Public, same reasoning as `driver-profile-pics` above.
    ...
    `service_role` grants confirmed working (2026-08-08) via the `driver-trip` Edge Function: `select`/`insert`/`update` on `sessions` (had to be granted — see `SUPABASE_GOTCHAS.md` #2/#7), plus `select` on `delivery_requests`/`driver_records`/`users`/`devices` (already sufficient, no grant needed). **`delete` was missing, 2026-08-14 through 2026-09-08** (`SUPABASE_GOTCHAS.md` #11) — every real Trip-lifecycle action only ever inserts/updates a session, so this was never granted and never needed until disposable test scripts started trying to delete one, twice. **Fixed 2026-09-08** (`supabase/migrations/20260908110000_grant_sessions_delete_service_role.sql`, `grant delete on public.sessions to service_role;`) — no real feature needs it yet either, granted proactively so it's no longer a recurring test-cleanup friction point.
    ...
    - manual_close_offset_meters (numeric, nullable) — **deployed 2026-09-13** (`supabase/migrations/20260913160000_sessions_manual_close_offset_meters.sql`, `IMPLEMENTATION/14B_ARRIVED_AT_BASE_CONFIRMATION.md`). Distance in meters from `WAREHOUSE_COORDS` at the moment `driver-trip`'s `end-return-trip` (the manual "Arrived at Base" fallback) closed this Session — computed from the driver's `livePosition` at tap time if supplied, else the session's most recent `gps_logs` row, else left `null` rather than fabricated. `null` for every Session not closed this way (auto geofence/timeout close, `start-trip`'s force-close, or not a return-trip Session at all); written on *every* `end-return-trip` close including from within the geofence (a small value), so presence alone marks "closed manually" while the stored number is what the UI compares against `RETURN_TRIP_GEOFENCE_METERS` (150m) to decide whether to show the "Closed {X}km from base" badge in the per-session breakdown — passive, visible only if a Supervisor opens that trip's report, never a live notification.
    ...
    - Also read by `crew_capacity_blocked_days(p_horizon_days int)` (`SECURITY DEFINER`, written and deployed 2026-09-26 via `supabase/migrations/20260926000000_crew_capacity_blocked_days.sql`), which `CustomerRequestDelivery.jsx` calls to grey out pick-up dates the customer genuinely can't book: returns the `'YYYY-MM-DD'` dates over the next `p_horizon_days` (default 365) where fewer than one free Driver or fewer than one free Helper remain — "free" = working that weekday here and not committed to an active `delivery_requests` trip spanning that date (`pickup_date` → `dropoff_date`, the same `CREW_ACTIVE_STATUSES` range `lib/crewStatus.js` uses). Customers with `crew_client_specialties` rows only get their specialized crew counted, matching what the Supervisor's Assign Vehicle picker can actually assign to them. Replaced the earlier client-side rule ("any date with an active booking is greyed out"), which blocked a day as soon as one booking existed without ever counting crew. Grant: `execute` to `authenticated` only, same pattern as `specialized_crew_available_days()`.
...
```
    ...
    **Write path extended 2026-09-09 — no longer Pi-exclusive.** Until this date every row came from the physical Raspberry Pi via `gps-upload` (device-secret authenticated). A new `driver-trip` action, `log-position` (Driver-JWT authenticated — a phone can't use the device-secret model), now also inserts rows directly, fed by `DriverDeliveries.jsx`'s existing phone-GPS `watchPosition` effect, throttled to ~1/s. `session_id` is resolved the same way either path already did (Active session or null while Paused). Phone GPS is now the *primary* source; the Pi's `gps-upload` path is unchanged and still writes here too, now acting as the fallback for whenever a phone isn't broadcasting — see `project_gps_source_split` (session memory) and `05_GPS_PIPELINE.md`'s correction note for the full reasoning/history. No new grant needed: `service_role` already had `insert` here (shared client, grants are per-table not per-Edge-Function).
    ...
    **Tightened 2026-08-30** (`20260830130000_harden_driver_default_assignments.sql`, `SUPABASE_GOTCHAS.md` #12). Originally built as an explicit copy of the trucks table's pre-hardening model — `for all to authenticated using(true) with check(true)` — per its own migration's comment. Confirmed live and exploitable via curl (an authenticated Driver token could insert/read/update/delete any row); replaced with a `current_user_role() = ANY (ARRAY['Admin','Supervisor'])`-gated policy, verified fixed via the same curl reproduction (insert now returns `403`, Postgres code `42501`).
    ...
    - pickup_time_end (time, nullable) — **deployed 2026-08-16** (`supabase/migrations/20260816095039_delivery_requests_pickup_time_window.sql`). Pickup Time is a customer-entered window, not a single instant: `pickup_time` is the window's start, this is its end. Nullable so pre-existing rows (created before this feature) keep rendering as a single time rather than being backfilled with an invented value — see `CustomerRequestDelivery.jsx`'s Pick Up Window fields. **2026-09-15**: this window is purely informational ("Open from -- to --"), not a scheduling constraint — the form no longer requires the end to be later than the start, or blocks submission on it (`getPickupWindowError`/`getDropoffWindowError`/the time-based half of `getScheduleErrors`/`getStopTimeError` were removed from `lib/deliveryOptions.js`; only `getDropoffDateError` (the date-ordering check) remains, since delivering before pickup is physically impossible rather than just an unusual window). **2026-09-26**: a second cross-window check was added on top — `getDropoffTimeError`, which on a same-day delivery rejects a drop-off window that starts before the pick-up window ends (reported case: pick up 2:00–4:30 PM, drop off 8:00 AM). Window order *within* a window is still not enforced.
    - dropoff_time_end (time, nullable) — **deployed 2026-08-30** (`supabase/migrations/20260830140000_delivery_requests_dropoff_time_window.sql`), same shape as `pickup_time_end` above. `dropoff_time` is the window's start, this is its end. **Same-day follow-up (delivery time-planning feature, see `delivery_mode` below)**: only meaningful in `SAME_DAY` mode, where the form labels both together as "Open From – To"; in `TWO_DAY` mode `dropoff_time` is repurposed as the Day 2 start instant (see below) and this field is hidden/unused (null) — the "window" concept doesn't apply to a day-start clock. Same 2026-09-15 change as `pickup_time_end` above: no longer validated as a blocking constraint.
    - delivery_mode (text, nullable) — **deployed 2026-08-30** (`supabase/migrations/20260830150000_delivery_requests_delivery_mode.sql`), per explicit user decision. `"SAME_DAY"` (pickup + all drop-offs same day) or `"TWO_DAY"` (pickup Day 1, all drop-offs Day 2) — an explicit customer-facing toggle in the Request Delivery form, feeding `scheduleSimulator.js`'s full delivery-time simulation (pickup/loading, every travel leg, unloading, rest breaks every `REST_INTERVAL_HOURS` of travel, meal breaks on crossing `LUNCH_THRESHOLD_MINUTES`/`DINNER_THRESHOLD_MINUTES`). In `TWO_DAY` mode, Day 1 is a fixed 30-minute pickup/loading block only (no depot/origin distinct from the pickup point itself, so nothing to drive on Day 1); Day 2's start instant reuses `dropoff_time` (not a new column) and covers all travel/unloading/rests/meals. Day1+Day2 combined must stay ≤ `MAX_TOTAL_DELIVERY_HOURS` (13) or submission is rejected. Nullable, not backfilled on pre-existing rows — same precedent as `pickup_time_end`/`dropoff_time_end`. See `IMPLEMENTATION/02_BOOKING_AND_TRIP_CREATION.md`'s "Delivery Time-Planning Calculation" section and `lib/scheduleSimulator.js`.
    ...
    - pickup_arrived_at / dropoff_arrived_at (timestamptz, nullable) — **deployed 2026-09-08** (`supabase/migrations/20260908120000_delivery_requests_arrival_timestamps.sql`), for the Supervisor's Trip Details "Arrival" per location. Stamped by a new, optional, Driver-owned "Arrived at Pickup"/"Arrived at Drop-off" action (`admin-users`'s `update-driver-delivery`, statuses `ARRIVED_PICKUP`/`ARRIVED_DROPOFF` — these two status values already existed in transition guards and the `truck_status_sync` trigger but had never actually been set by any UI action until this). Skippable — Helper's Confirm Pickup/Complete Dropoff work identically whether or not the Driver tapped Arrived first. Pickup + primary Drop-off only, no per-stop equivalent (deliberate scope decision). Null on any delivery completed before this date or where the Driver skipped the button — rendered as "not recorded," never fabricated.
