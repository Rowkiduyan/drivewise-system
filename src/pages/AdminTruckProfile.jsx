...
// getPmsStatus is used internally by maintenance utilities as a fallback.
// Direct import not needed here.
import {
...
function AdminTruckProfile() {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    return localStorage.getItem("adminTruckActiveTab") || "overview";
  });
  // Persist active tab changes to localStorage
  useEffect(() => {
    localStorage.setItem("adminTruckActiveTab", activeTab);
  }, [activeTab]);
  ...
  // Log Maintenance Service Modal state
  ...
  if (autoStatusKey !== statusKey) {
    ...
    // 2. Else if the start date is **after** today → Scheduled.
    // 3. Otherwise (start today or past, with end date today/future) → In Progress.
  ...
  useEffect(() => {
    ...
    // Update the record to Completed and set the end date to today.
    supabase
      .from("maintenance_records")
      .update({
        status: "Completed",
      ...
      .then(async ({ error }) => {
        if (error) {
          ...
          // Refresh maintenance records first.
          await loadMaintenanceRecords();
          // Update truck's previous mileage and maintenance date based on the completed record.
          const today = new Date().toISOString().split("T")[0];
          supabase
            .from("trucks")
            .update({
            ...
            .eq("id", truck.id)
            .then(() => {
              // Re-fetch the truck to get updated fields for PMS calculation.
              supabase
                .from("trucks")
                .select("*")
                .eq("id", truck.id)
  ...
  // Compute latest record once for reuse
                  ...
                  >
                    ...
                    <div className="flex gap-3">
                      <button
                        type="button"
                        onClick={() => {
                          // Close modal and reset all input fields to defaults
                          setIsLogMaintenanceModalOpen(false);
                          setLogDate(new Date().toISOString().split("T")[0]);
                          setLogEndDate("");
                          setLogMileage("");
