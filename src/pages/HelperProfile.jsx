  ...
  return (
    <div>
      ...
      <input
        ...
        className="mt-1.5 w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 text-xs text-slate-900 outline-none transition focus:border-teal-400 focus:bg-white focus:ring-2 focus:ring-teal-100 sm:py-2.5 sm:text-sm"
      />
    </div>
  );
}
...
function HelperProfile() {
  ...
  const handleProfilePictureChange = async (event) => {
    ...
    event.target.value = "";
    if (!file || !helper?.id || isUpdatingPicture) {
      return;
    }
  ...
  const handleRemoveProfilePicture = async () => {
    if (!helper?.id || isUpdatingPicture) {
      return;
    }
    ...
    try {
      const { error } = await supabase.functions.invoke("admin-users", {
        body: { action: "remove-profile-picture", userId: helper.id },
      });
  ...
  return (
    <HelperLayout title="Helper Profile" background={null}>
        ...
        {isLoadingProfile ? (
          ...
          <>
            {/* Profile header — horizontal strip matching the width and card
                ...
                No role badge here: this is the Helper portal, so "Helper"
                next to the helper's own name is redundant. */}
          ...
          >
            ...
            <form onSubmit={handleChangePassword} className="mt-3 flex flex-col gap-3 sm:mt-4 sm:gap-4">
              ...
              <div className="mt-1 flex items-center justify-end gap-2">
                ...
                <button
                  type="submit"
                  className="rounded-xl bg-teal-600 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-teal-700 sm:px-3.5 sm:py-2 sm:text-sm"
                >
                  Update Password
                </button>
              </div>
