...
const MAINTENANCE_TYPES = [
  ...
  "Battery Check", "Transmission Service", "Air Filter Replacement",
];

const MAINTENANCE_SHOPS = [
  "Fleet Care Manila", "AutoWorks Cavite", "Isuzu Service Center QC",
  "Hino Service Pasig", "TruckFix Alabang",
];
...
function createSeededRng(idString) {
  let state = 0;
  for (let i = 0; i < idString.length; i += 1) state = (state * 31 + idString.charCodeAt(i)) >>> 0;
  if (state <= 0) state = 1;
...
function buildMockMaintenance(truck) {
  ...
  return Array.from({ length: recordCount }, (_, i) => {
    ...
    if (status === "Overdue") {
      ...
      actualDate = new Date(NOW.getTime() + (i + 1) * (5 + rng() * 10) * DAY_MS);
    } else {
      actualDate = new Date(NOW.getTime() - i * (12 + rng() * 18) * DAY_MS);
    }

    const daysFromNow = Math.round((actualDate.getTime() - NOW.getTime()) / DAY_MS);
    ...
    const odometer =
      status === "Completed"
        ? Math.max(0, Math.round(((truck.odometer - Math.max(0, -daysFromNow) * (80 + rng() * 70)) / 10)) * 10)
        : Math.round(((truck.odometer + Math.max(0, daysFromNow) * (60 + rng() * 40)) / 10)) * 10;

    return {
      id: `MTN-${3100 - i}`,
      date: actualDate,
      dateLabel: actualDate.toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" }),
...
function getMaintenanceUrgency(record) {
  ...
  if (status === "Overdue") {
    return { label: daysFromNow === 0 ? "Due today" : `${Math.abs(daysFromNow)}d overdue`, tone: "rose" };
  }
  if (status === "Scheduled") {
    if (daysFromNow <= 7) {
      return { label: daysFromNow === 0 ? "Due today" : `Due in ${daysFromNow}d`, tone: "amber" };
    }
...
function MaintenanceStatusBadge({ status }) {
  return (
    <span
      className={`inline-flex shrink-0 items-center whitespace-nowrap rounded-full px-2.5 py-1 text-xs font-semibold ${
        MAINTENANCE_STATUS_BADGE_CLASSES[status] || MAINTENANCE_STATUS_BADGE_CLASSES.Scheduled
      }`}
    >
      {status}
    </span>
...
function StatTile({ label, icon: Icon, tone = "slate", children }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-center gap-2">
        <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ${TONE_ICON_CLASSES[tone]}`}>
...
function SupTruckProfile() {
  ...
  const maintenanceRecords = useMemo(() => (truck ? buildMockMaintenance(truck) : []), [truck]);

  const upcomingMaintenance = useMemo(
    () => maintenanceRecords.filter((record) => record.status !== "Completed").sort((a, b) => a.date - b.date),
    [maintenanceRecords],
  );
  const completedMaintenance = useMemo(
    () => maintenanceRecords.filter((record) => record.status === "Completed").sort((a, b) => b.date - a.date),
    [maintenanceRecords],
  );
  ...
  const filteredMaintenance =
    maintenanceStatusFilter === "All"
      ? maintenanceRecords
      : maintenanceRecords.filter((record) => record.status === maintenanceStatusFilter);
  ...
  const maintenanceStatusCounts = {
    ...
    Overdue: maintenanceRecords.filter((record) => record.status === "Overdue").length,
  };
        ...
        {activeTab === "overview" && (
          <SectionCard title="Truck Information">
            <div>
              ...
              <InfoRow label="Assigned Driver" value={truck.assignedDriver || "Unassigned"} />
        ...
        {activeTab === "maintenance" && (
          <div className="flex flex-col gap-4">
            ...
            <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              <StatTile label="Condition" icon={overallCondition.icon} tone={overallCondition.tone}>
                <p className={`text-base font-bold ${TONE_TEXT_CLASSES[overallCondition.tone]}`}>
              ...
              <StatTile label="Next Service" icon={Calendar} tone={nextService ? getMaintenanceUrgency(nextService).tone : "slate"}>
                {nextService ? (
                  <>
                    <p className="truncate text-sm font-bold text-slate-900">{nextService.type}</p>
                    <p className="mt-0.5 truncate text-xs text-slate-500">
                      {nextService.dateLabel} · {getMaintenanceUrgency(nextService).label}
                    </p>
                  </>
                ) : (
                  <p className="text-sm font-semibold text-slate-500">None scheduled</p>
                )}
              </StatTile>
              ...
              <StatTile label="Last Service" icon={CheckCircle2} tone="emerald">
                {lastService ? (
                  <>
                    <p className="truncate text-sm font-bold text-slate-900">{lastService.type}</p>
                    <p className="mt-0.5 truncate text-xs text-slate-500">
                      {lastService.dateLabel} · {lastService.odometer.toLocaleString()} km
                    </p>
                  </>
                ) : (
                  <p className="text-sm font-semibold text-slate-500">No history</p>
                )}
              </StatTile>

              <StatTile label="Since Last Service" icon={Gauge} tone="blue">
                <p className="text-sm font-bold text-slate-900">
                  {(lastService ? Math.max(0, truck.odometer - lastService.odometer) : truck.odometer).toLocaleString()} km
                </p>
                <p className="mt-0.5 text-xs text-slate-500">Now at {truck.odometer.toLocaleString()} km</p>
              </StatTile>
            </section>
              ...
              {upcomingMaintenance.length === 0 ? (
                ...
                <ul className="flex flex-col divide-y divide-slate-100">
                  {upcomingMaintenance.map((record) => {
                    const TypeIcon = MAINTENANCE_TYPE_ICONS[record.type] || Wrench;
                    return (
                      <li key={record.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-sm font-semibold text-slate-900">{record.type}</p>
                          <p className="truncate text-xs text-slate-500">
                            {record.shop} · {record.dateLabel} · ~{record.odometer.toLocaleString()} km
                          </p>
                        </div>
                    ...
                    {filteredMaintenance.map((record) => {
                      ...
                      return (
                        <tr key={record.id} className="transition hover:bg-slate-50">
                          ...
                          <td className="px-5 py-4 text-slate-700">{record.dateLabel}</td>
                          ...
                          <td className="px-5 py-4 text-slate-700">{record.odometer.toLocaleString()} km</td>
                          <td className="px-5 py-4 text-slate-700">{record.shop}</td>
                          <td className="px-5 py-4">
