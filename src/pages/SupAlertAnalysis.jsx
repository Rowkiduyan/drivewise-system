...
function SupAlertAnalysis() {
  ...
  const {
    ...
    alerts.forEach((item) => {
      ...
      if (item.session_id) {
        if (!alertsBySession[item.session_id]) {
          alertsBySession[item.session_id] = { count: 0, last: item.created_at }
        }
    ...
    const typesRows = Object.keys(ALERT_TYPE_LABELS).map((key) => {
      ...
      const share = totalAlerts ? `${Math.round((count / totalAlerts) * 100)}%` : '0%'
    ...
    const sessionRows = Object.entries(alertsBySession)
      .map(([sessionId, info]) => {
        ...
        const status = total >= 10 ? 'Escalate' : total >= 5 ? 'Review' : 'Monitor'
        return {
          sessionId,
          alerts: total,
    ...
    return {
      kpis: [
        { label: 'Total alerts (7d)', value: String(totalAlerts), hint: 'Across all sessions' },
        ...
        { label: 'Peak window', value: formatPeakWindow(peakEntry.hour), hint: 'Most alerts by hour' }
      ],
    ...
    <SupLayout title="Alert Analysis" background={null} bg="bg-[#FAF9F6]">
      <div className="flex flex-col gap-6">
        <header className="space-y-2">
          ...
          <h1 className="font-display text-3xl font-semibold sm:text-4xl">Alert Analysis</h1>
          <p className="max-w-2xl text-sm text-slate-600">
            Explore alert patterns by type, time window, and driver risk to prioritize interventions.
          </p>
        </header>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {kpis.map((k) => (
            <StatCard key={k.label} label={k.label} value={isLoading ? '...' : k.value} hint={k.hint} />
          ))}
        </div>
                ...
                <tbody className="divide-y divide-slate-200">
                  {alertTypes.map((row) => (
                    <tr key={row.type} className="bg-white">
                      ...
                      <td className="px-4 py-3 text-slate-700">{isLoading ? '...' : row.count}</td>
                      <td className="px-4 py-3 text-slate-700">{isLoading ? '...' : row.share}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <Panel title="Alerts by Time" right="Hour buckets">
            <div className="space-y-3">
              ...
              <p className="pt-1 text-xs text-slate-500">
                Use this view to align breaks, swaps, and coaching with high-frequency windows.
              </p>
              ...
              <tbody className="divide-y divide-slate-200">
                {(isLoading ? [] : flaggedSessions).map((row) => (
                  <tr key={row.sessionId} className="bg-white">
                    <td className="px-4 py-3 font-medium text-slate-900">{row.sessionId}</td>
                    <td className="px-4 py-3 text-slate-700">{row.alerts}</td>
                    <td className="px-4 py-3 text-slate-700">{formatDuration(row.duration)}</td>
                    <td className="px-4 py-3 text-slate-700">{formatTimestamp(row.last)}</td>
