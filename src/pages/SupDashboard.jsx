...
function KPIHero({ items }) {
  return (
    <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
      {items.map((it) => (
        <div key={it.title} className="flex items-center gap-4 rounded-2xl bg-white p-4 shadow-sm">
          <div className={`h-12 w-2 rounded-full ${it.color}`} />
          <div className="flex-1">
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium text-slate-600">{it.title}</p>
              <p className={`text-sm font-semibold ${it.delta >= 0 ? 'text-green-600' : 'text-red-500'}`}>{it.delta >= 0 ? `▲ ${it.delta}%` : `▼ ${Math.abs(it.delta)}%`}</p>
            </div>
            <p className="mt-2 text-2xl font-bold text-slate-900">{it.value}</p>
            {it.meta && <p className="text-xs text-slate-500 mt-1">{it.meta}</p>}
          </div>
        </div>
      ))}
    </div>
...
function SupDashboard() {
  const heroItems = [
    { title: 'Drowsiness Alerts', value: '128', delta: 5, color: 'bg-amber-400' },
    { title: 'High-Risk Drivers', value: '6', delta: 0, color: 'bg-red-400' },
    { title: 'On-Time Delivery', value: '92%', delta: 1.2, color: 'bg-emerald-400' },
    { title: 'Total Deliveries', value: '1,234', delta: -0.3, color: 'bg-sky-400' }
  ]
  ...
  return (
    <SupLayout title="Supervisor Dashboard" background={background} bg="bg-[#FAF9F6]">
          ...
          <SummaryCard title="Fleet & Drivers">
            <div className="grid gap-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold">{summary.trucks.available}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-slate-700">In Transit</span>
                <span className="font-semibold">{summary.trucks.inTransit}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-slate-700">Under Maintenance</span>
                <span className="font-semibold">{summary.trucks.maintenance}</span>
              </div>
              <div className="border-t mt-3 pt-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold">{summary.drivers.active}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-slate-700">Currently In Trip</span>
                  <span className="font-semibold">{summary.drivers.inTrip}</span>
                </div>
              </div>
            </div>
          </SummaryCard>
            ...
            <ul className="space-y-2">
              {recentAlerts.map((a) => (
                <li key={a.id} className="flex items-center justify-between">
                  ...
                  <div className="text-xs text-red-500 font-semibold">Alert</div>
                </li>
              ))}
            </ul>
          </SummaryCard>
