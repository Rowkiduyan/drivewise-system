...
import imutils 
import numpy as np
import threading # NEW: For background GPS reading
import serial    # NEW: To communicate with the GPS module
...
def parse_gps_nmea(line):
    ...
    try:
        # Check for GPRMC sentence (Recommended Minimum Specific GPS/TRANSIT Data)
        # It contains both valid status and lat/lon data
        parts = line.split(',')
        if line.startswith('$GPRMC') and len(parts) >= 7 and parts[2] == 'A': # 'A' means Active/Valid signal
            lat_raw = parts[3]
            lat_dir = parts[4]
            lon_raw = parts[5]
            lon_dir = parts[6]
        ...
        pass # Silently ignore malformed GPS lines

def gps_thread_function():
    try:
        # Connect to Pi's hardware serial port (/dev/serial0)
...
# UPDATED: Now inserts latitude and longitude into the alerts table!
...
try:
    while True:
        ...
        for rect in rects:
            ...
            # NEW: GPS STATUS ON SCREEN
            gps_status = f"GPS: {current_lat}, {current_lon}" if current_lat != 0.0 else "GPS: Searching..."
            cv2.putText(frame, gps_status, (10, 220), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
